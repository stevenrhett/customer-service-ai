...
from app.utils.exceptions import (AgentError, ConfigurationError, SessionError,
                                  VectorStoreError)
...
async def test_agent_error_exception():
    ...
    error = AgentError(
        "Agent processing failed", agent_name="billing", details={"agent": "billing"}
    )
...
async def test_chat_endpoint_handles_agent_error(mock_env_vars):
    ...
    with patch("app.api.chat.get_orchestrator") as mock_get_orch:
        ...
        mock_orchestrator.process_query = AsyncMock(
            side_effect=AgentError("Agent failed", agent_name="billing")
        )
        mock_get_orch.return_value = mock_orchestrator

        async with AsyncClient(app=app, base_url="http://test") as client:
            response = await client.post("/api/chat", json={"message": "Test message"})
    ...
    with patch("app.api.chat.get_orchestrator") as mock_get_orch:
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            response = await client.post(
                "/api/chat/stream", json={"message": "Test message"}
            )
...
async def test_validation_error_handling(mock_env_vars):
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        response = await client.post("/api/chat", json={"message": 123})
        assert response.status_code == 422
